...
class Solution:
    def totalCost(self, costs: List[int], k: int, candidates: int) -> int:
        ...
        for i in range(candidates):
            heapq.heappush(candidates_queue, [costs[i], i, 1])
            p1 += 1
        for i in range(candidates):
            if p2 >= p1:
                heapq.heappush(candidates_queue, [costs[p2], p2, -1])
                p2 -= 1

        total_cost = 0
        for _ in range(k):
            cost, _, direction = heapq.heappop(candidates_queue)
            total_cost += cost

            # at some point p1 will catch up the p2
            # prevent add candidates using p1 index, because they are already added
            if p1 <= p2:
                if direction == 1:
                    heapq.heappush(candidates_queue, [costs[p1], p1, 1])
                    p1 += 1
                else:
                    heapq.heappush(candidates_queue, [costs[p2], p2, -1])
                    p2 -= 1
