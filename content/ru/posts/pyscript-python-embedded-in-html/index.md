...
_Кнопка `Install` здесь для шутки, так как установка не тербуется_
...
Разработчики PyScript пишут, что для работы не треубется никакой среды разработки, кроме веб-браузера. Попробуем запустить в Chrome.
...
Тег `<py-script>` расположен внутри HTML body. Внутри этого тега будем пиать Python код.
